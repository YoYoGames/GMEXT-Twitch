...
/// 
/// Note: This function is adapted from a community member, JujuAdams (2021-09-16).
/// 
...
/// @param {Bool} _dispose - Whether to resize or create a new buffer (default is false).
/// @param {Bool} _return_string - Whether to return the hash as a hexadecimal string or an array of words (default is true).
/// @returns {String|Array<Real>} The resulting hash, either as a hexadecimal string or an array of words.
/// @ignore
function __twitch_buffer_sha256(_in_buffer, _in_offset = 0, _size = (buffer_get_size(_in_buffer) - _in_offset), _dispose = false, _return_string = true)
{
	// Constants
    ...
    {
        _buffer = buffer_create(__sha256_block_size*_block_count, buffer_fixed, 1);
        _offset = 0;
    ...
    if (_return_string)
    {
        ...
        repeat(8)
        {
            switch(_i)
            {
                case 0: _value = _state_array_0; break;
                case 1: _value = _state_array_1; break;
                case 2: _value = _state_array_2; break;
                case 3: _value = _state_array_3; break;
                case 4: _value = _state_array_4; break;
                case 5: _value = _state_array_5; break;
                case 6: _value = _state_array_6; break;
                case 7: _value = _state_array_7; break;
            }
...
/// @returns {Array<Real>} The resulting HMAC-SHA256 hash as an array of 32-bit words.
/// @ignore
function __twitch_hmac_sha256(_key_buffer, _message)
{
    ...
    if (_key_length > _block_size)
    {
        ...
        var _hash = __twitch_buffer_sha256(_key_buffer, undefined, undefined, true, false);
    ...
    var _hash = __twitch_buffer_sha256(_inner_pad_buffer, 0, buffer_tell(_inner_pad_buffer), true, false);
    ...
    return __twitch_buffer_sha256(_outer_pad_buffer, 0, buffer_tell(_outer_pad_buffer), true, false);
}
...
/// @ignore
function __twitch_buffer_base64_url_encode(_buffer) {
...
/// @returns {String} The Base64Url encoded string.
/// @ignore
function __twitch_base64_url_encode(_str) {
...
/// @desc Converts a word array (32-bit words) into a binary buffer.
/// This function takes an array of 32-bit words and writes each word into a buffer as 4 bytes in big-endian order.
/// It then returns the buffer that can be used for further binary operations.
/// @param {Array<Real>} _word_array The array of 32-bit words to be converted into a binary buffer.
/// @returns {Id.Buffer} The resulting buffer containing the binary data.
/// @ignore
function __twitch_word_array_to_binary_buffer(_word_array) {
	var _len = array_length(_word_array);
	// Create a buffer to hold the 32 bytes (8 words x 4 bytes per word)
	var _buffer = buffer_create(_len * 4, buffer_fixed, 1);  // 32 bytes buffer
	...
	for (var _i = 0; _i < _len; _i++) {
		...
		buffer_write(_buffer, buffer_u8, (_word >> 24) & 0xff);  // Most significant byte
		buffer_write(_buffer, buffer_u8, (_word >> 16) & 0xff);
		buffer_write(_buffer, buffer_u8, (_word >> 8) & 0xff);
		buffer_write(_buffer, buffer_u8, _word & 0xff);          // Least significant byte
	}

	// Read the buffer as a binary string
	buffer_seek(_buffer, buffer_seek_start, 0);
...
function twitch_extensions_create_jwt_token(_secret, _payload = {}) {
	...
	var _signature_word_array = __twitch_hmac_sha256(_secret_buffer, _data);
	
	// 6. Convert word array to binary buffer and encode it (base64 url)
	var _signature_binary_buffer = __twitch_word_array_to_binary_buffer(_signature_word_array);	
