 ...
 * @param {struct} optionals The optional parameters to be passed into the function:
 * 
 * - `emote_mode` : ${type.bool} : A Boolean value that determines whether chat messages must contain only emotes.
 *   Set to `true` if only emotes are allowed; otherwise, `false`. The default is `false`.
 * - `follower_mode` : ${type.bool} : A Boolean value that determines whether the broadcaster restricts the chat room to followers only.
 ...
 * - `follower_mode_duration` : ${type.bool} : The length of time, in minutes, that users must follow the broadcaster before being able to participate in the chat room. Set only if `follower_mode` is `true`. Possible values are: 0 (no restriction) through 129600 (3 months). The default is 0.
 * - `non_moderator_chat_delay` : ${type.bool} : A Boolean value that determines whether the broadcaster adds a short delay before chat messages appear in the chat room. This gives chat moderators and bots a chance to remove them before viewers can see the message.
 ...
 * - `non_moderator_chat_delay_duration` : ${type.real} : The amount of time, in seconds, that messages are delayed before appearing in chat. Set only if `non_moderator_chat_delay` is `true`. Possible values are:
 *   * 2 - 2 second delay (recommended)
 *   * 4 - 4 second delay
 *   * 6 - 6 second delay
 * - `slow_mode` : ${type.bool} : A Boolean value that determines whether the broadcaster limits how often users in the chat room are allowed to send messages. Set to `true` if the broadcaster applies a wait period between messages; otherwise, `false`. The default is `false`.
 *   To specify the delay, see the `slow_mode_wait_time` field.
 * - `slow_mode_wait_time` : ${type.real} : The amount of time, in seconds, that users must wait between sending messages. Set only if `slow_mode` is `true`.
 *   Possible values are: 3 (3 second delay) through 120 (2 minute delay). The default is 30 seconds.
 * - `subscriber_mode` : ${type.bool} : A Boolean value that determines whether only users that subscribe to the broadcaster’s channel may talk in the chat room.
 *   Set to `true` if the broadcaster restricts the chat room to subscribers only; otherwise, `false`. The default is `false`.
 * - `unique_chat_mode` : ${type.bool} : A Boolean value that determines whether the broadcaster requires users to post only unique messages in the chat room.
